...
export interface SeerrImportStats {
  users: number;
  usersCreated: number;
  imported: number;
  updated: number;
...
const SETTINGS_URL_KEY = 'seerr_url';
const SETTINGS_API_KEY = 'seerr_api_key';
...
/**
 * Import Seerr (Jellyseerr / Overseerr) requests into Fliks's `FliksRequest`
 * table.
 *
 * Matching strategy:
 ...
 */
@Injectable()
export class SeerrRequestImportService {
  private readonly log = new Logger(SeerrRequestImportService.name);
  ...
  async importFromSeerr(): Promise<SeerrImportStats> {
    const url = await this.settings.get(SETTINGS_URL_KEY);
    const apiKey = await this.settings.get(SETTINGS_API_KEY);
    if (!url || !apiKey) {
      throw new BadRequestException('Configure Seerr URL and API key first');
    }

    const stats: SeerrImportStats = {
      users: 0,
      usersCreated: 0,
      imported: 0,
      updated: 0,
    ...
    // Cache (type:tmdbId) -> title fetched from Seerr's TMDB proxy. Same
    ...
    let skip = 0;
    while (true) {
      const { results, total } = await this.seerr.listRequests(
        url,
        apiKey,
        skip,
        pageSize,
      ...
      for (const req of results) {
        try {
          ...
          stats.errors.push(`request #${req.id}: ${(err as Error).message}`);
          this.log.warn(
            `Seerr import: request #${req.id} skipped — ${(err as Error).message}`,
          );
        }
      }
    ...
    this.log.log(
      `Seerr import: ${stats.users} user(s) (${stats.usersCreated} created), ${stats.imported} imported, ${stats.updated} updated, ${stats.skipped} skipped, ${stats.errors.length} error(s)`,
    );
    return stats;
  }
  ...
  private async applyRequest(
    ...
    const mediaType = req.type === 'tv' ? MediaType.SERIES : MediaType.MOVIE;
    ...
    //   2. Seerr's request payload (rarely populated — only when the
    //      build exposes title/name on the embedded media object)
    //   3. Seerr's TMDB proxy `/api/v1/{movie|tv}/{tmdbId}` — covers
    //      orphans (no Fliks Media yet). Cached by (type, tmdbId) so
    //      multi-user requests for the same title only hit Seerr once.
    let title = media?.title ?? req.media?.title ?? req.media?.name ?? '';
    if (!title) {
      let cached = titleByKey.get(key);
      if (cached === undefined) {
        cached = await this.seerr.fetchTitle(
          seerrUrl,
          seerrApiKey,
          req.type,
          tmdbId,
    ...
    // Preserve Seerr's authoritative timestamps so the imported request
    // looks identical in age/listing order to its source — falling back to
    // "now" only when Seerr returned an unparseable value.
    ...
    if (existing) {
      ...
      // Restore Seerr's timestamps after `save` (TypeORM auto-stamps
  ...
  private resolveUsername(u?: SeerrUser): string | null {
    if (!u) return null;
    const candidate =
  ...
  private async resolveUser(
    ...
    if (!user) {
      ...
      this.log.log(
        `Seerr import: created Fliks user "${username}" (role=${defaultRole?.name ?? 'none'})`,
      );
    }
  ...
  private mapStatus(
    requestStatus: SeerrMediaRequest['status'],
    mediaStatus?: SeerrMediaRequest['media']['status'],
  ): RequestStatus {
    ...
    if (mediaStatus === SEERR_MEDIA_STATUS.AVAILABLE) {
      return RequestStatus.AVAILABLE;
    }
    if (
      mediaStatus === SEERR_MEDIA_STATUS.PROCESSING ||
      mediaStatus === SEERR_MEDIA_STATUS.PARTIALLY_AVAILABLE
    ) {
