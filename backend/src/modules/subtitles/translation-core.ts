...
/** Shared per-run inputs common to every engine. */
...
/** Some engines (Gemini) report the wait for a 429 in the error body's RetryInfo
 *  (e.g. "38s") rather than the Retry-After header — parse it so per-minute
 *  quota bursts recover instead of failing on the short default backoff. */
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    ...
    try {
      ...
      if (attempt >= MAX_ATTEMPTS) {
        throw new Error(
          `${engineLabel} request failed after ${MAX_ATTEMPTS} attempts: ${String(err)}`,
        );
      }
...
/**
 * The LLM prompt is deliberately terse to save tokens: it carries the
 * constraints and a little media context, and asks for numbered segments back so
 * the timings never need to travel to the model.
 */
export function buildSystemInstruction(req: TranslationRequest): string {
  const { context } = req;
...
export function buildPayload(texts: string[]): string {
