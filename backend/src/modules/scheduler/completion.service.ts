...
import { QbittorrentService, QbittorrentTorrent } from '../download-clients/qbittorrent.service';
...
export class CompletionService {
  ...
  async processCompleted(): Promise<void> {
    ...
    const allTorrents = (
      await Promise.all(qbitClients.map((c) => this.qbittorrent.getTorrents(c)))
    ).flat();
    ...
    const completedTorrents = allTorrents.filter(
      (t) => t.progress >= 1 || t.state === 'seeding' || t.state === 'stalledUP',
    );
