...
export const SUITARR_LANGUAGES: SuitarrLanguageDefinition[] = [
...
const byId = new Map(SUITARR_LANGUAGES.map((l) => [l.id, l]));

export function getSuitarrLanguageById(id: number): SuitarrLanguageDefinition | undefined {
  return byId.get(id);
}

export const UNKNOWN_LANGUAGE = SUITARR_LANGUAGES.find((l) => l.id === 99)!;
export const ENGLISH_LANGUAGE = SUITARR_LANGUAGES.find((l) => l.id === 1)!;
